...
PixelArray *new_pixel_array(size_t width, size_t height, char *name) {
  ...
  pixel_array->vertices =
      (uint32_t *)calloc(2 * MAX_VERTICES, sizeof(uint32_t));
...
inline size_t get_pixel_array_width(const PixelArray *pixel_array) {
  return pixel_array->width;
}

inline size_t get_pixel_array_height(const PixelArray *pixel_array) {
...
inline uint32_t unsafe_get_pixel(const PixelArray *pixel_array, uint32_t x,
                                 uint32_t y) {
...
uint32_t get_pixel(const PixelArray *pixel_array, uint32_t x, uint32_t y) {
  ...
  __ERROR_CHECK(x > pixel_array->width, errcode = 1, "X-CORDERR",
                "Out-of-bounds X array access max %lu: provided %u",
                pixel_array->width, x)
  __ERROR_CHECK(y > pixel_array->height, errcode = 1, "Y-CORDERR",
                "Out-of-bounds Y array access max %lu: provided %u",
...
inline void unsafe_set_pixel(PixelArray *pixel_array, uint32_t x, uint32_t y,
                             uint32_t color) {
  pixel_array->pixels[x + pixel_array->width * y] = color;
}

void set_pixel(PixelArray *pixel_array, uint32_t x, uint32_t y,
               uint32_t color) {
  int errcode = 0;
  __ERROR_CHECK(x > pixel_array->width, errcode = 1, "X-CORDERR",
                "Out-of-bounds X array access max %lu: provided %u",
...
void copy(const PixelArray *src, PixelArray *dest, uint32_t sx, uint32_t sy,
  ...
  __ERROR_CHECK(dy + sh > dest->height, errcode = 1, "DESTERR",
                "Out-of-bounds Y paste : max %lu, dy = %u, sh = %u",
                dest->height, dy, sh)
  __ERROR_CHECK(dx + sw > dest->width, errcode = 1, "DESTERR",
                "Out-of-bounds X paste : max %lu, dx = %u, sw = %u",
                dest->width, dx, sw)
  __ERROR_CHECK(sy + sh > src->height, errcode = 1, "SRCERR",
                "Out-of-bounds Y copy : max %lu, sy = %u, sh = %u", src->height,
                sy, sh)
  __ERROR_CHECK(sx + sw > src->width, errcode = 1, "SRCERR",
                "Out-of-bounds X copy : max %lu, sx = %u, sw = %u", src->width,
                sx, sw)

#if DEBUG_BUILD
  printf("\x1b[1;34m[SRCDBG][INFO]> X max %lu, sx = %u, sw = %u, sum = "
         "%u\x1b[0m\n",
         src->width, sx, sw, sx + sw);
  printf("\x1b[1;34m[SRCDBG][INFO]> Y max %lu, sx = %u, sw = %u, sum = "
         "%u\x1b[0m\n",
         src->height, sy, sh, sy + sh);
  printf("\x1b[1;34m[DSTDBG][INFO]> X max %lu, sx = %u, sw = %u, sum = "
         "%u\x1b[0m\n",
         dest->width, dx, sw, dx + sw);
  printf("\x1b[1;34m[DSTDBG][INFO]> Y max %lu, sx = %u, sw = %u, sum = "
         "%u\x1b[0m\n",
