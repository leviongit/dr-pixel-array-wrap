...
// --- Rectangles :
void stroke_rectangle(PixelArray *pixel_array, uint32_t x, uint32_t y,
...
void fill_rectangle(PixelArray *pixel_array, uint32_t x, uint32_t y,
                    uint32_t width, uint32_t height, uint32_t color) {
...
void scan_polygon_left_edge(PixelArray *pixel_array, uint32_t x1, uint32_t y1,
...
void scan_polygon_right_edge(PixelArray *pixel_array, uint32_t x1, uint32_t y1,
                             uint32_t x2, uint32_t y2) {
  int32_t x, y, dx, dy, x_increment, y_increment, d;
...
void fill_polygon(PixelArray *pixel_array, uint32_t color) {
  ...
  uint32_t previous_point_x =
      pixel_array->vertices[2 * ((max_y_index - 1 + pixel_array->vertex_count) %
                                 pixel_array->vertex_count)];
  uint32_t next_point_x =
      pixel_array
          ->vertices[2 * ((max_y_index + 1) % pixel_array->vertex_count)];
  int32_t winding_order = previous_point_x < next_point_x ? -1 : 1;

  // Left :
  i = 0;
  while (pixel_array->vertices[2 * (((max_y_index + i * winding_order) +
                                     pixel_array->vertex_count) %
                                    pixel_array->vertex_count) +
                               1] != min_y) {
    scan_polygon_left_edge(
        pixel_array,
        pixel_array->vertices[2 * (((max_y_index + i * winding_order) +
                                    pixel_array->vertex_count) %
                                   pixel_array->vertex_count)],
        pixel_array->vertices[2 * (((max_y_index + i * winding_order) +
                                    pixel_array->vertex_count) %
                                   pixel_array->vertex_count) +
                              1],
        pixel_array->vertices[2 * (((max_y_index + (i + 1) * winding_order) +
                                    pixel_array->vertex_count) %
                                   pixel_array->vertex_count)],
        pixel_array->vertices[2 * (((max_y_index + (i + 1) * winding_order) +
                                    pixel_array->vertex_count) %
  ...
  for (i = min_y; i <= max_y; i += 1)
    draw_horizontal_line(pixel_array, pixel_array->left_scan[i],
                         pixel_array->right_scan[i], i, color);
}
